        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                @endif
                    ...
                    @if($bancosArray)
                        @foreach($bancosArray as $dadosBanco)
                            <div class="card mt-3" id="formulario-pergunta">

                                <div class="card-header">
                                    <h4 class="card-title">Informe os emails de {{ $dadosBanco->nomeCompleto }}</h4>
                                </div>
                                <div class="card-body">
                                    <form action="{{ route('email.store') }}" class="form-group" method="post">
                                        @csrf
                                        <input id="idClienteTelefone" value="{{ $dadosBanco->id }}" name="cliente_id" type="hidden">
                                        <input id="tkClienteTelefone" value="{{ $dadosBanco->uid }}" name="uid" type="hidden">
