                        ...
                        @foreach($cliente as $newCliente)
                            <div class="card mt-3" id="formulario-pergunta">
                                ...
                                <div class="card-header">
                                    <h4 class="card-title">Informe os telefones de {{ $newCliente->nomeCompleto }}</h4>
                                </div>
                                <div class="card-body">
                                    <form action="/telefone/store/" class="form-group" method="post">
                                        @csrf
                                        <input id="idClienteTelefone" value="{{ $newCliente->id }}" name="cliente_id" type="hidden">
                                        <input id="tkClienteTelefone" value="{{ $newCliente->uid }}" name="uid" type="hidden">
